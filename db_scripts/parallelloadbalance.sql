...
CREATE OR REPLACE FUNCTION public.parallelsqlload(db text, table_to_chunk text, pkey text, query text, output_table text, table_to_chunk_alias text default '', num_procs integer default 2, replacement_string text default '1=1',blocksize integer default 1000)
  RETURNS text AS
$BODY$
DECLARE 
  sql     TEXT;
...
BEGIN
  ...
  --the step_size is determaint by the block size
  step_size := blocksize;
