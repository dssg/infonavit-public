...
$RUN_ON_MYDB <<SQL
SELECT DropGeometryColumn('','denue2013','geom');
DROP TABLE if exists denue2013;
CREATE TABLE denue2013 (gid serial,
"d_llave" int4,
"cve_ent" varchar(2),
"entidad" varchar(40),
"cve_mun" varchar(3),
...
"latitud" numeric,
"longitud" numeric);
ALTER TABLE denue2013 ADD PRIMARY KEY (gid);
SELECT AddGeometryColumn('','denue2013','geom','4326','POINT',2);
SQL
...
find $DIR  -type f -name denue_converted.shp | sort | sed '$d' | xargs -n 1 -i sh -c 'echo "File:"$0; shp2pgsql -a -s 4326 -W "latin1" $0 denue2013 | psql -X -U infonavit -h dssgsummer2014postgres.c5faqozfo86k.us-west-2.rds.amazonaws.com -d infonavit ' {} | grep -v "INSERT 0 1"

#add the last and create the index
find $DIR  -type f -name denue_converted.shp | sort | tail -n 1 | xargs -n 1 -i sh -c 'echo "File:"$0; shp2pgsql -a -I -s 4326 -W "latin1" $0 denue2013 | psql -X -U infonavit -h dssgsummer2014postgres.c5faqozfo86k.us-west-2.rds.amazonaws.com -d infonavit ' {} | grep -v "INSERT 0 1"
