...
do
$RUN_ON_MYDB <<SQL
drop table if exists feature_geo_distance_naics_denue$YEAR;
SQL

TABLE=$(cat create_feature_geo_distance_naics_denue2010.sql | sed "s/denue2010/denue$YEAR/g" )
$RUN_ON_MYDB -c "$TABLE"

STATEMENT=$(cat naics_distance.sql | sed "s/denue2010/denue$YEAR/g")
...
(	'infonavit',				--database
	...
	'feature_geo_distance_naics_denue$YEAR',--result table, has to be created first
